...
class opDiaryPluginDiaryActions extends opDiaryPluginActions
{
  ...
  {
    $publicFlag = $this->isSnsMember() ? PluginDiaryTable::PUBLIC_FLAG_SNS : PluginDiaryTable::PUBLIC_FLAG_OPEN;
  ...
  public function executeListMember(sfWebRequest $request)
  {
    if (!$this->isSNSMember())
    {
      $this->forwardUnless($this->member && Doctrine::getTable('Diary')->hasOpenDiary($this->member->getId()),
    ...
    $this->pager = Doctrine::getTable('Diary')->getMemberDiaryPager($this->member->getId(), $request->getParameter('page'), 20, $this->myMemberId, $this->year, $this->month, $this->day);
  }
  ...
  {
    if (PluginDiaryTable::PUBLIC_FLAG_OPEN != $this->diary->getPublicFlag() && !$this->isSnsMember())
    {
